...
for uid_index, uid in enumerate(survey3.iloc[:,0]) :
    ...
    for i in range(len(X)) :
        if X.iloc[i, 0] == np.nan :
            X.iloc[i, 24] = np.nan # hour
            X.iloc[i, 25] = np.nan # minute
...
#model.add(LSTM(50, input_shape=(a_timesteps, a_features), dropout=0.2, recurrent_dropout=0.2))
...
## Fit network
history = model.fit(train_X, train_Y, epochs=30, batch_size=a_batch_size, validation_data=(test_X, test_Y), shuffle=False)
